...
st.markdown('<span style="font-size:14px; color:#FFFFFF">Matthew began his career using legacy proprietary tools developed decades ago. This early exposure to the inefficiencies and tech debt of legacy systems, as well as ever-increasing work scope, prompted Matthew to embrace new tools as available.  This began with leveraging SAS to handle most incoming analysis and reporting work with an emphasis on file processing and management to avoid the manual processes used by other teams in the department. His team happily adopted this mindset and successfully decreased processing times across the board. A notable accomplishment was the reduction of 3-4 weeks from the processing time of an externally published annual report.  As data visualizations tools became more prominent, Matthew leaped at the chance to stand up Power BI as the reporting platform for Burlington Stores. He also simultaneously supported the transition from on-premise to a cloud data warehouse, Snowflake. The Power BI reports that he and the team delivered dramatically improved the speed with which the business got results.  What previously could take hours was now available within seconds while also allowing slicing and dicing that was not previously possible. Matthew moved to build on that success by joining a startup subsidiary within Horizon Blue Cross Blue Shield named NovaOne Health. At NovaOne, he contributed to the development of various data products within the dbt semantic layer built on top of AWS Redshift and provided insights via AWS Quicksuite. He joined just as the NovaOne began operations with its first customer and fielded questions from the business and the customer about the user base and claims activity.  </span>', unsafe_allow_html=True)

st.markdown('<span style="font-size:22px; color:#DAF7DC">SKM to eSKM Migration</span>', unsafe_allow_html=True)
...
st.markdown('<span style="font-size:22px; color:#DAF7DC">GENASYS to PARcore</span>', unsafe_allow_html=True)
